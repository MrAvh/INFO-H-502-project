...
public:
	...
	MeshObject();
	MeshObject(char const *obj_file, char const *texture_file, int num, Shader shader, glm::vec3 position = glm::vec3(0.0, 0.0, 0.0));
...
private:
	...
	GLuint createVAO(std::vector<glm::vec3> vertices, std::vector<glm::vec2> uvs, std::vector<glm::vec3> normals);
